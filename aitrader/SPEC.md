...
### `POST /api/buy`
成交前**再过一次硬风控**（硬拦 409）。LIVE+私钥→真实发单（按 `chain` 取 adapter/原生币/精度/钱包）；否则 SHADOW 只记录 + 落 positions.json，记 `chain`/`entry_price`。
```json
请求: { "address":"...", "size_sol":0.01, "chain":"sol" }
返回: { "ok":true, "status":"confirmed · <txhash> | SHADOW（…）", "symbol":"..." }
```
